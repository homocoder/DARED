...
def usage():
	...
	print("\t\tredPipeSD --mous=uid://A002/X5ce05d/X6b\n")
	print("\t\tredPipeSD --mous=uid://A002/X5ce05d/X6b --env=/opt/sacm/etc/c4r2\n")
...
def main():
	...
	p.intents   = p.redPipeSD_intents
	p.procedure = p.redPipeSD_procedure
	p.run('step_get_xtss_mous_state')	
	p.run('step_log_shell_environment')
	p.run('step_get_ppr')

	p.mous_fuid = p.mous_uid.replace("/", "_").replace(":", "_")
	p.identifier = p.directory.split('_')[0].replace('.', '_') + '.' + p.mous_fuid + '.' + p.directory[15:].replace('.', '_') + '.redPipeSD'
	...
	p.run('step_execute_ppr_from_ms_redPipeSD')
	...
	p.run('step_move_products_to_spool_area')
	p.run('step_da_permissions')

	p.l('redPipeSD has finished :)')

	#p.mous_state = 'ReadyForReview'
	#p.mous_state_transition_msg = dict()
	#p.mous_state_transition_msg['tstamp'] = datetime.now().isoformat()
	#p.mous_state_transition_msg['step'] = 'redPipeSD'
	#p.mous_state_transition_msg['hint'] = 'redPipeSD has finished :)'
