...
def usage():
	...
	print("\t\tredPipeIF --mous=uid://A002/X5ce05d/X6b\n")
	print("\t\tredPipeIF --mous=uid://A002/X5ce05d/X6b --env=/opt/sacm/etc/c4r2\n")
...
def main():
	...
	p.intents   = p.redPipeIF_intents
	p.procedure = p.redPipeIF_procedure
	p.run('step_get_xtss_mous_state')	
	p.run('step_log_shell_environment')
	p.run('step_get_ppr')

	p.mous_fuid = p.mous_uid.replace("/", "_").replace(":", "_")
	p.identifier = p.directory.split('_')[0].replace('.', '_') + '.' + p.mous_fuid + '.' + p.directory[15:].replace('.', '_') + '.redPipeIF'
	...
	p.run('step_execute_ppr_from_ms_redPipeIF')
	...
	p.l('redPipeIF has finished :)')

	#p.mous_state = 'ReadyForReview'
	#p.mous_state_transition_msg = dict()
	#p.mous_state_transition_msg['tstamp'] = datetime.now().isoformat()
	#p.mous_state_transition_msg['step'] = 'redPipeIF'
	#p.mous_state_transition_msg['hint'] = 'redPipeIF has finished :)'
